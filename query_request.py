...
class SSLAdapter(HTTPAdapter):
    def init_poolmanager(self, *args, **kwargs):
        ...
        context.set_ciphers('TLS_AES_128_GCM_SHA256')  # 强制使用 TLS 1.3
...
def query(date, from_station, to_station):
    ...
    headers = {
        ...
        'Cookie': '_uab_collina=173503278306764628454386; JSESSIONID=D7470D92B5652A412F73A49200A02CB5; BIGipServerportal=3168010506.16671.0000; BIGipServerpassport=870842634.50215.0000; guidesStatus=off; highContrastMode=defaltMode; cursorStatus=off; route=9036359bb8a8a461c164a04f8f50b252; _jc_save_fromStation=%u5317%u4EAC%2CBJP; _jc_save_fromDate=2024-12-24; _jc_save_toDate=2024-12-24; _jc_save_wfdc_flag=dc; BIGipServerotn=1440284938.50210.0000; _jc_save_toStation=%u798F%u5DDE%2CFZS'
    }
    # 使用自定义 SSLAdapter 强制使用 TLS 1.3
...
def query_time(station):
    ...
    stations = eval(read('time.text'))  # 读取所有车站并转换为dic类型(字典类型）
    ...
    for i in data:  # 遍历查询车站所对应的所有站名
        part_before_comma = i.split(',')[1]
        if part_before_comma in stations:  # 在站名时间文件中，判断是否存在该站名
            station_name_list.append(part_before_comma)  # 有该站名就将站名添加至列表中
